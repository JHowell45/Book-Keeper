...
class LoginRefresh(APIView):
    def post(self, request):
        try:
            ...
            try:
                decoded_refresh_token_payload = decode(
                    refresh_token, settings.SECRET_KEY, algorithms="HS256"
                )
            except InvalidSignatureError:
                return Response(
                    {"error": "Invalid Signature, Token tampered!"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                return Response(
                    {"error": "Token tampered!"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            try:
                ...
                return Response(
                    {"error": "Fatal! Multiple users with the same " "user name exist"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            print(er)
            return Response(
                "Oops!, Some thing went wrong while handling your request",
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
